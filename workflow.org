...
spread(dunnett_mean_corrected, key=Treatment, value=Mean_expr) %>%
    dplyr::select(Gene, C, B, BS, L, N, P, S) %>%
    write_tsv("sig_pway_collage_expression.tsv", col_names=FALSE)
...
xls <- read_excel("ChlamEE_140218_stoich_growthrates_resp.xlsx") %>%
    mutate(Ancestor = gsub(" ", "_", Ancestor))
names(xls) <- names(xls) %>% gsub(" ", "_", .)
xls <- mutate(xls, Ratio = (2*Cass) / Respiration,
             Selection_Treatment = factor(Selection_Treatment,
                                          levels=c("A", "C", "L", "N",
                                                   "P", "B", "S", "BS"))) %>%
    gather(variable, value, Cass_Chl, Respiration_Chl, Ratio)

xls_wilcox <- read_excel("ChlamEE_140218_stoich_growthrates_resp.xlsx") %>%
    mutate(Ancestor = gsub(" ", "_", Ancestor))
...
xls_cell_size <- read_excel("ChlamEE_dataset_12072018_updated.xlsx") %>%
    mutate(Ancestor = gsub(" ", "_", Ancestor))
names(xls_cell_size) <- names(xls_cell_size) %>% gsub(" ", "_", .)
xls_cell_size <- mutate(xls_cell_size,
                       Selection_Treatment = factor(Selection_Treatment,
                                          levels=c("A", "C", "L", "N",
                                                   "P", "B", "S", "BS"))) %>%
    gather(variable, value, Cell_diameter, Cell_Volume,
           SD_Diameter, Cell_density, Total_Biovolume)

xls_cell_size_wilcox <- read_excel("ChlamEE_dataset_12072018_updated.xlsx") %>%
    mutate(Ancestor = gsub(" ", "_", Ancestor))
names(xls_cell_size_wilcox) <- names(xls_cell_size_wilcox) %>% gsub(" ", "_", .)
xls_cell_size_wilcox <- mutate(xls_cell_size_wilcox,
                       Selection_Treatment = factor(Selection_Treatment,
                                          levels=c("A", "C", "L", "N",
                                                   "P", "B", "S", "BS")))
...
get_wilcox_density <- function(treatment, stoich)
{
    control <- filter(xls_cell_size_wilcox, Selection_Treatment == "A")[[stoich]]
    tmt <- filter(xls_cell_size_wilcox, Selection_Treatment == treatment)[[stoich]]
    tidy(wilcox.test(x=control, y=tmt)) %>%
        mutate(Treatment = treatment, Stoich = stoich) %>%
        select(-method, -alternative, -statistic)
}
...
map_df(tmts, ~get_wilcox_density(., "Total_Biovolume"))
...
**** R
#+BEGIN_SRC R :session
xls <- read_excel("ChlamEE_140218_stoich_growthrates.xlsx")
names(xls) <- names(xls) %>% gsub(" ", "_", .)
xls$Ancestor <- xls$Ancestor %>% gsub(" ", "_", .)

for_r1 <- dplyr::select(xls, Ancestor, Selection_Treatment, Biomass, NO3, NH4, PO4) %>%
...
pdf("biomass.pdf", useDingbats = FALSE)
ggplot(for_r1, aes(x=Selection_Treatment, y=Biomass, color=Selection_Treatment)) +
    geom_boxplot() +
    geom_point(aes(size=2)) +
    theme_bw() + 
    theme(strip.text.y = element_text(angle = 0),
...
pdf("no3tobiomass.pdf", useDingbats = FALSE)
ggplot(for_r1, aes(x=Selection_Treatment, y=NO3toBiomass, color=Selection_Treatment)) +
    geom_boxplot() +
    geom_point(aes(size=2)) +
    theme_bw() + 
    theme(strip.text.y = element_text(angle = 0),
...
pdf("po4tobiomass.pdf", useDingbats = FALSE)
ggplot(for_r1, aes(x=Selection_Treatment, y=PO4toBiomass, color=Selection_Treatment)) +
    geom_boxplot() +
    geom_point(aes(size=2)) +
    theme_bw() + 
    theme(strip.text.y = element_text(angle = 0),
