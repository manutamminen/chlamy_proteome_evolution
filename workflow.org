...
adj_p_vals <- function(p_table) sapply(p.adjust.methods, function(meth) p.adjust(p_table$p.value, meth))
add_p_vals <- function(p_table) cbind(p_table, adj_p_vals(p_table))
adjust_wilcoxon <- function(column, hypothesis) map_df(tmts, ~wilcoxon_test(., column, hypothesis)) %>% add_p_vals
