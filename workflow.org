...
pathview(gene_data=pw_matrix,
         pathway_id = "00710",
         out_suffix = "Photos_fixation")
pathview(gene_data=pw_matrix_organelles,
         pathway_id = "00710",
         out_suffix = "Photos_fixation_org")
pathview(gene_data=pw_dunnett_matrix_organelles,
         pathway_id = "00710",
         out_suffix = "Photos_fixation_dunnett_org")
pathview(gene_data=sig_matrix,
         pathway_id = "00710",
...
pathview(gene_data=pw_matrix,
         pathway_id = "00910",
         out_suffix = "Nitrogen")
pathview(gene_data=pw_dunnett_matrix,
         pathway_id = "00910",
         out_suffix = "Nitrogen_dunnett")
#+END_SRC


* Stoichiometry and Respiration; Supplementary Figs. 8 and 9
...
xls <- read_excel("ChlamEE_phenotypic_measurements.xlsx") %>%
    mutate(Ratio = (2*Cass) / Respiration,
           Selection_Treatment = factor(Selection_Treatment,
                                        levels=c("A", "C", "L", "N",
                                                 "P", "B", "S", "BS")))
xls_long <- gather(xls, key=variable, value=value, Respiration, Ratio,
                  Cass, CtoP_Molar, CtoN_Molar, Cass_Chl, Respiration_Chl)
...
bind_rows(map_df(tmts, ~get_wilcox_growth(., "CtoN_Molar")),
          map_df(tmts, ~get_wilcox_growth(., "CtoP_Molar")),
          map_df(tmts, ~get_wilcox_growth(., "Respiration")),
